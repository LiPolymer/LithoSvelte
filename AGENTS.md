...
- Tooltip uses Tonal colors and backdrop blur. Pointer hover is delayed,
  ...
  trigger center.
- Tooltip keeps requested `open`, measured `positioned`, and visual `revealed`
  state separate. Never reveal it until FloatingLayer has produced a valid
  position for the current trigger. Pointer activation dismisses the hint and
  suppresses the focus event generated by that activation; touch pointers do
  not open hover hints, while genuine keyboard focus still opens immediately.
- After a pointer-opened Tooltip closes, nearby Tooltips may skip their normal
  hover delay for the short `skipDelay` window. Escape consumes the key only
  when it actually cancels a pending or visible Tooltip, so a parent Dialog or
  Menu is not dismissed at the same time.
